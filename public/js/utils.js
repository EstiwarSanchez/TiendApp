...
function validateFile(
    target,
    extension = [true, /\.(jpe?g|png|jpg)$/i],
    size = [true, 2]
) {
    var error = 0;
    ...
    for (let file of files) {
        if (extension[0] && !error) {
            error = checkExtension(file,extension[1]) ? 1 : 0;
        }
        if (size[0] && !error) {
            error = checkSize(file, size[1]) ? 2 : 0;
        }
    }
...
function checkExtension(file, extRegex = /\.(jpe?g|png|jpg)$/i) {
    if (extRegex.test(file.name) === false) {
        return true;
    }
    return false;
}
    ...
    return {
        ...
        submitForm(e) {
            ...
            if (invalidElements.length > 0) {
                ...
                this.$el.addEventListener('submit', evn=>{
                    evn.preventDefault();
                    evn.stopPropagation();
                });
                runEvent('submit',`#${this.$el.id}`);
            }
        },
    ...
    return {
        ...
        init(json = null) {
            ...
            if (json != null) {
                ...
                setTimeout(() => {
                    this.saveFormat()
                    // getParentX('form_builder').saveFormat();
                }, 200);
        ...
        drop(event, id) {
            this.id =
                localStorage.getItem("format_template_id") != undefined &&
                localStorage.getItem("format_template_id") != "undefined"
                    ? localStorage.getItem("format_template_id")
                    : id;
        ...
        defineElement(element, id) {
            ...
            if (
                ...
                // console.log(element,element.querySelectorAll(".input-label"));
            ...
            if (type == "radio") {
                element.querySelector('.validate-msg').setAttribute('data-validate',element.id)
                element.querySelector('.validate-msg').setAttribute("x-show.transition.in",`${element.id}.errorMessage`);
                element.querySelector('.validate-msg').setAttribute("x-text",`${element.id}.errorMessage`);
            }
        ...
        saveFormat() {
            setTimeout(() => {
                ...
                form.querySelectorAll('input, select, textarea').forEach(input => {
                    ...
                    var validate= document.querySelector(`[data-validate="${input.type == 'radio' ? input.name : input.id}"]`);
...
function callEvent(event, querySelector) {
    ...
    if (isElement(querySelector)) {
        ...
        e.initEvent(event, true, event == 'submit' ? true :false);
...
function proccessOptions(id, options, instance, build = false, preselect = false) {
    ...
    select.innerHTML = `<option value="" disabled selected>${LANG["Select an option"]}</option>`;
    ...
    if (options.length > 0) {
        options.forEach(element => {
            if (!selectedValues.includes(String(element.id))|| !preselect) {
                element.text = element.text.replace(/(<([^>]+)>)/gi, "")
...
Livewire.on('searchSelect', (params) => {
    if (
        params[0].id == "company_id" ||
        params[0].id == "company"
    ) {
        var url = `${__PATH__}/companies`;
        var query = {
            search: params[1],
            limit: 10,
        };
        getOptionsByParams(params[0].id, url, "description", "id", query);
    }
    ...
    if (
        ...
        var url = `${__PATH__}/roles`;
        var query = {
            search: params[1],
            limit: 10,
        };
        getOptionsByParams(params[0].id, url, "name", "name", query);
    }
})
            ...
            .then(function (response) {
                ...
                response.data.data.forEach((item) => {
                    ...
                    if (Array.isArray(text)) {
                        text.forEach((t,i) => {
                            txt += (LANG[item[t]] ?? item[t])+ ((i+1)>=text.length ? '' : ' - ')
                        });
                    }else{

                        txt = LANG[item[text]] ?? item[text]
                    }

                    if (Array.isArray(val)) {
                        val.forEach((t,i) => {
                            vl += (LANG[item[t]] ?? item[t])+ ((i+1)>=val.length ? '' : '-')
                        });
                    }else{
                        vl = item[val]
                    }
                ...
                proccessOptions(id, data, instance, build, true);
            })
            .catch(function (error) {
                console.log(error);
            });
...
function initAttrs(ready = false){
    if (ready) {
        inputs = [];
        document.querySelectorAll('input, textarea').forEach((element) => {
            var existId = element.id != "";
            if (existId) {
                inputs.push({
                    id: element.id,
